...
const baseConvChangelog = 'conventional-changelog --config scripts/conventional-changelog/';
...
function getChangelog(shell) {
  const changelog = shell.exec(`${baseConvChangelog} -u`, {silent: true}).trim().split('\n');

...
function releaseCanaryVersion() {
  shell.echo(`Clinia JS Helper release CANARY version`);
  ...
  updateCanaryVersion(canaryVersion => {
    ...
    revertStableVersion(packageJson.version, () => {
      shell.echo(`Clinia JS Helper v${canaryVersion} released`);
    });
  });
}

function releaseStableVersion() {
  shell.echo(`Clinia JS Helper release STABLE version`);
  ...
  promptVersion(packageJson.version, (version) => {
    bumpVersion(version, () => {
      ...
      shell.echo(`Clinia JS Helper v${version} released`);
    });
  });
}
